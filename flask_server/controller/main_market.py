...
def _find_code(input_code_normalized, market, lang):
    ...
    for lang_suffix in ['ko', 'en']:
        ...
        if len(matched_indices) == 1:
            ...
            potential_matches.extend(df_names.loc[matched_indices, [name_column, 'code']].values.tolist())
    ...
    if len(matched_indices) == 1:
        ...
        potential_matches.extend(df_names.loc[matched_indices, ['name_ko', 'code']].values.tolist())
...
def _check_in_another_market(input_code_normalized, another_market):
    ...
    for col_name in ['name_ko', 'name_en', 'code']:
        uppercased_values = another_market_names[col_name].str.replace(' ', '').str.upper()
    ...
    for col_name in ['name_ko', 'name_en', 'code']:
        ...
        if len(matched_indices) == 1:
            ...
            potential_matches.extend(another_market_names.loc[matched_indices, col_name].values.tolist())
...
def _handle_post_request(request, market):
    ...
    if code_found:
        ...
        if _check_in_another_market(input_code_normalized, another_market):
            error_key = "코스피 & 코스닥 검색기를 이용해주세요." if another_market == 'kospi_daq' else "뉴욕증권 & 나스닥 검색기를 이용해주세요."
        else:
            error_key = "해당 종목명 또는 종목코드가 존재하지 않거나 지원하지 않는 종목입니다."
