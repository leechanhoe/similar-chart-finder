...
def pattern_search():
    ...
    # 이상한 값 처리
    if not validate_market(market, PAGE_NAME) or not validate_code(
        code, market, PAGE_NAME
    ):
    ...
    stock_data = get_stock_data_pre_fol(
        code, base_date, market, preceding=17, following=14
    )
    ...
    after_close_change_5, after_close_change_10 = _get_after_close_change_5_10(
        stock_data, base_date
    )
    first_pattern_day = stock_data.index[
        stock_data.index.get_loc(base_date) - 3
    ].strftime("%Y-%m-%d")
...
def _get_after_close_change_5_10(stock_data, base_date):
    ...
    if base_index + 5 < len(close_prices):
        after_5 = (
            (close_prices[base_index + 5] - close_prices[base_index])
            / close_prices[base_index]
            * 100
        )
    if base_index + 10 < len(close_prices):
        after_10 = (
            (close_prices[base_index + 10] - close_prices[base_index])
            / close_prices[base_index]
            * 100
        )
