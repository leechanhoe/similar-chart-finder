...
from pyfile.data_reader import get_name, get_naver_url, get_same_industry_code, get_valid_day_num, get_stock_code
...
def stock_info():
    ...
    if market is None:
        ...
        if code in kospi_daq_codes:
            ...
            return render_template('none_stock_info_invalid_code.html', code=code, market=market,
                                      translations=translations[lang], lang=lang, popular_ranking=get_popular_ranking(market, with_name=lang))
    ...
    if not validate_code(code, market, PAGE_NAME):
        if code in get_stock_code(market, only_code=True, only_valid=False):
            return render_template('none_stock_info.html', code=code, name=get_name(code, market, lang), market=market,
                                      translations=translations[lang], lang=lang, popular_ranking=get_popular_ranking(market, with_name=lang))
        else:
            return render_template('none_stock_info_invalid_code.html', market=market,
                                      translations=translations[lang], lang=lang, popular_ranking=get_popular_ranking(market, with_name=lang))
    ...
    if data['data_num_allday'] > 0:
        data['rise_rate_allday'] = round(data['rise_count_allday'] / data['data_num_allday'] * 100, 1)
    else:
...
def _get_same_industry(code, market, lang):
    ...
    same_industry['name'] = same_industry['code'].apply(get_name, args=(market, lang, ))
...
def get_statistics_data(code, market, lang, past_date=''):
    ...
    data[cols_to_round] = data[cols_to_round].round(2) # redis에 넣다 빼면 데이터 손실이 있어 다시 반올림
