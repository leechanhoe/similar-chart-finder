...
from pyfile.profit_validation import (
    get_validation_allday,
    get_all_validation_date,
    is_verified_date,
)
...
def validation():
    ...
    total_pages = (
        len(detail_data) + ITEMS_PER_PAGE - 1
    ) // ITEMS_PER_PAGE  # 전체 페이지 수 계산
    # 페이지 범위 계산
    ...
    template_kwargs = {
        ...
        "popular_ranking": get_popular_ranking(
            "kospi_daq" if lang == "ko" else "nyse_naq", with_name=lang
        ),
...
def detail():
    ...
    if profit_validation is None:
        logging.info(
            f"validation/detail error - date : {date} / code : {code} / lang : {lang}"
        )
        return main.page_not_found404(lang=lang)
    ...
    if statistics_data is None:
        logging.info(
            f"validation/detail no statistics_data - date : {date} / code : {code} / lang : {lang}"
        )
    ...
    if statistics_data["data_num_allday"] > 0:
        statistics_data["rise_rate_allday"] = round(
            statistics_data["rise_count_allday"]
            / statistics_data["data_num_allday"]
            * 100,
            1,
        )
    else:
    ...
    verified_date, d_day = is_verified_date(date)
    if verified_date:
        draw_plt_stock_chart(
            code,
            date,
            lang,
            get_stock_data_pre_fol(code, date, market, preceding=31, following=10),
            validation_chart=True,
        )
    draw_all_after_change_chart(
        code, market, lang, statistics_data, validation_date=date
    )
    ...
    logging.info(
        f"visited {PAGE_NAME}/detail - date={date} / code={code} / lang={lang}"
    )
