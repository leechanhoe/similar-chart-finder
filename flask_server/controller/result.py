...
from pyfile.similar_data_reader import (
    get_similar_chart,
    get_one_comparison_result,
    get_similar_data_range,
)
from pyfile.data_reader import get_latest_update_date
from translation import translations
from popular_ranking import get_popular_ranking
from validation_value import (
    validate_lang,
    validate_market,
    validate_day_num,
    validate_code,
...
def result():
    ...
    # 이상한 값 오류처리
    if (
        not validate_market(market, PAGE_NAME)
        or not validate_code(code, market, PAGE_NAME)
        or not validate_day_num(day_num, PAGE_NAME)
        or not validate_date_format(base_date)
    ):
    ...
    similar_chart_data["compare_stock_name"] = similar_chart_data[
        "compare_stock_code"
    ].apply(
        get_name,
        args=(
    ...
    similar_chart_data["compare_stock_industry"] = similar_chart_data[
        "compare_stock_code"
    ].apply(
        get_industry,
        args=(
    ...
    logging.info(
        f"visited {PAGE_NAME} : ({get_name(code, market, lang)}){code} {base_date} {day_num} mean : {after_close_mean}"
    )
...
def detail():
    ...
    if (
        not validate_market(market, PAGE_NAME)
        or not validate_day_num(day_num, PAGE_NAME)
        or not validate_code(base_stock_code, market, PAGE_NAME)
        or not validate_code(compare_stock_code, market, PAGE_NAME)
    ):
        return main.page_not_found404(lang=lang)

    if not validate_date_format(base_date) or not validate_date_format(compare_date):
        logging.info(
            f"{PAGE_NAME} invalid access - base_date : {base_date} / compare_date : {compare_date}"
        )
        return main.page_not_found404(lang=lang)

    day_num = int(day_num)
    result = get_one_comparison_result(
        base_stock_code, base_date, compare_stock_code, compare_date, day_num, market
    )
    if len(result) != 1:  # db에 저장된 비교결과에 없으면
        return main.page_not_found404(lang=lang)
    compare_stock_name = get_name(compare_stock_code, market, lang)

    detail_chart = draw_detail_chart(
        compare_stock_code, compare_date, market, day_num, lang
    )

    logging.info(
        f"visited detail_chart : (base: {get_name(base_stock_code, market, lang)}){base_stock_code} {base_date} {day_num} / compare : {get_name(compare_stock_code, market, lang)}){compare_stock_code}"
    )
    template_kwargs = {
        "base_date": base_date,
...
def get_param(param_name):
    return request.args.get(param_name) or request.args.get(
