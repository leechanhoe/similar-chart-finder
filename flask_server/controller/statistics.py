...
def statistics():
    ...
    if (
        market not in get_market_list()
        or up not in [True, False]
        or rise_rate not in [True, False]
        or day_num not in get_day_num_list(type="str") + ["all"]
    ):
        logging.info(
            f"{PAGE_NAME} invalid access - market : {market} / up : {up} / rise_rate : {rise_rate} / day_num : {day_num}"
        )
