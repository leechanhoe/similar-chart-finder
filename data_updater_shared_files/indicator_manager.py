...
def calculate_consecutive(stock_data):
    ...
    last_consecutive = last_consecutive[
        ~last_consecutive["Group"].isin(volume_zero_groups)
    ]
    # 연속성이 1 이상인 행만 필터링
...
def update_indicator(market, start_time, period=120):
    ...
    for i, code in enumerate(code_list):
        ...
        for indicator_name, indicator_func in indicators.items():
            if (
                indicator_name == "ConsecutiveCandles"
                or indicator_name == "ConsecutiveTrends"
            ):
                ...
                agg_results = aggregate_indicator_results(
                    data.copy(), indicator_values, indicator_name
                )
    ...
    for indicator_name in final_results:
        ...
        combined_agg = combined_results.groupby(level=0).agg(
            {"Average_Return": "mean", "Positive_Count": "sum", "Total_Count": "sum"}
        )
        combined_agg["Average_Return"] = combined_agg["Average_Return"] * 100
        combined_agg["Positive_rate"] = (
            combined_agg["Positive_Count"] / combined_agg["Total_Count"] * 100
        )
    ...
    for indicator_name in final_results.keys():
        final_results[indicator_name].to_csv(
            f"/app/shared_files/shared_data/indicator_{market}_{indicator_name}.csv"
        )
