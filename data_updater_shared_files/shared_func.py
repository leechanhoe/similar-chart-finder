...
from pyfile.data_reader import update_cache_code_name_industry_url, get_comparable_code_list, get_up_down_ranking, get_same_industry_code
...
def load_index_data(start_date, market):
    ...
    while attempt < max_attempts:
        try:
            ...
            data['Open'] = data.apply(lambda row: row['Low'] if row['Open'] < row['Low'] else row['Open'], axis=1)

            # High보다 Close나 Open이 높은 경우를 찾아 High로 설정합니다.
            data['Close'] = data.apply(lambda row: row['High'] if row['Close'] > row['High'] else row['Close'], axis=1)
            data['Open'] = data.apply(lambda row: row['High'] if row['Open'] > row['High'] else row['Open'], axis=1)
...
def update_valid_stock_code(market, base_on_ranking=10000):
    delete_invalid_comparison_result(market) # valid를 업데이트하기 전이나 후나 한번이라도 invalid가 됐으면 삭제
    update_valid_requested_stock_code(market) # 관리자가 특정 코드를 (in)validate하라는 요청이 있는지 확인 후 실행
    update_valid_base_on_ranking(market, base_on_ranking) # 시총 순위가 ranking 이하인 종목은 유효한 종목이라고 판단
    update_cache_code_name_industry_url(market) # 위 2 함수로 valid가 업데이트 된 것을 레디스 캐시에 반영
...
def update_valid_base_on_ranking(market, cutline=10000):
    ...
    with engine.begin() as connection:
        ...
        query = text(f"SELECT code, ranking FROM stock_code_list_{market} WHERE {cutline} < ranking AND valid = 1 AND user_request = 0") # 유저 요청종목은 제외
        delist = pd.read_sql(query, engine)
        delist = delist[~delist['code'].isin(get_compared_code_list(market))] # 비교 테이블에 없는 행만 삭제
        logging.info(f"number of delist : {len(delist)}")

        update_query = text(f"UPDATE stock_code_list_{market} SET valid = 0 WHERE code=:code")
        for idx, row in delist.iterrows():
            connection.execute(update_query, {"code": row['code']})
            logging.info(f"{row['code']}'s ranking is {row['ranking']}. valid is updated to 0")

        #상승하여 시총 상위 커트라인 위로 올라온 종목은 추가
        query = text(f"SELECT code, ranking FROM stock_code_list_{market} WHERE ranking <= {cutline} AND valid = 0 AND user_request = 0")
        enlist = pd.read_sql(query, engine)
        logging.info(f"number of enlist : {len(enlist)}")

        update_query = text(f"UPDATE stock_code_list_{market} SET valid = 1, failed_to_load = 0 WHERE code=:code")
        for idx, row in enlist.iterrows():
            connection.execute(update_query, {"code": row['code']})
            logging.info(f"{row['code']}'s ranking is {row['ranking']}. valid is updated to 1")

        if market == 'kospi_daq':
            query = text(f"""UPDATE stock_code_list_{market} SET valid = 0 WHERE code IN 
...
def update_valid_requested_stock_code(market):
    ...
    query = text(f"""SELECT t1.*
                        ...
                        ORDER BY t1.date;""") # code 별로 가장 최신 날짜의 행만을 가져오는 쿼리
    requested_stock_df = pd.read_sql(query, engine)

    with engine.begin() as connection:
        update_stock_code_list = text(f"""UPDATE stock_code_list_{market} 
                                        SET valid = :valid, user_request = 1
                                        WHERE code = :code""")

        for idx, row in requested_stock_df.iterrows():
            connection.execute(update_stock_code_list, {'valid': row['validate'], 'code': row['code']})
            logging.info(f"{row['code']}'s valid is updated to {row['validate']}")
        connection.execute(text(f"UPDATE requested_stock_code_{market} SET applied=1 WHERE applied=0"))
...
def delete_invalid_comparison_result(market, delist=None):
    ...
    with engine.begin() as connection:
        if delist is None:
            delist = pd.read_sql(text(f"SELECT code FROM stock_code_list_{market} WHERE valid=0"), engine)
        logging.info(f"number of delist : {len(delist)}")

        valid_code_list = get_compared_code_list(market)
        for code in delist['code'].tolist():
            if code in valid_code_list: # conparison 테이블에 비교당한 종목으로 있으면 삭제하지 않음
                continue
            
            deleted_num = 0
            for day_num in get_day_num_list():
                delete_comparison_result = text(f"DELETE FROM comparison_result_{day_num}day_{market} WHERE base_stock_code=:code")
                deleted_num += connection.execute(delete_comparison_result, {"code": code}).rowcount

            if deleted_num > 0:
                logging.info(f"{code}'s comparison_result are Deleted - num : {deleted_num}")