...
def _market_open_day(market, start_time):
    ...
    update_stock_data(start_time, market) # 맨 초기 db에 아무것도 없을 땐 recreate=True로 해줘야 메모리누수 안남
...
def _market_close_day(market, start_time):
    if datetime.today().weekday() == 5: # 토요일이면
        update_valid_stock_code(market, base_on_ranking=10000 if market == 'kospi_daq' else 1000) # (in)valid 종목 리스트 업데이트
