...
def update_cache(market, up, rise_rate, day_num):
    ...
    # 데이터베이스에서 데이터를 가져옵니다.
    data = get_statistics(
        market, up=up, rise_rate=rise_rate, day_num=day_num, update=True
    )
...
def update_statistics(base_date, market, save_period=36):
    ...
    for i, df in enumerate(df_list):  # enumerate를 사용하여 i 인덱스를 추가
        ...
        result_df.columns = [
            "code",
            f"average_{2**(i+3)}day",
        ]  # 'after_close_change' 컬럼 이름을 변경
    ...
    final_df["average_allday"] = final_df[
        [f"average_{day}day" for day in get_day_num_list()]
    ...
    columns_to_fill = [
        [f"rise_count_{day}day" for day in get_day_num_list()]
        + [f"data_num_{day}day" for day in get_day_num_list()]
    ]
    for column in columns_to_fill:
        # object 타입으로 저장된 데이터에서 숫자를 추론하여, 이를 정수로 변환
        final_df[column] = final_df[column].fillna(0).infer_objects().astype(int)
    final_df["rise_count_allday"] = final_df[
        [f"rise_count_{day}day" for day in get_day_num_list()]
    ].sum(axis=1, skipna=False)
    final_df["data_num_allday"] = final_df[
        [f"data_num_{day}day" for day in get_day_num_list()]
    ].sum(axis=1, skipna=False)
    ...
    with engine.begin() as connection:
        ...
        old_date = (
            pd.to_datetime(base_date).date() - relativedelta(months=save_period)
        ).strftime("%Y-%m-%d")
        delete_query = text(
            f"DELETE FROM statistics_{market} WHERE date < '{old_date}'"
        )
        connection.execute(delete_query)

        # 최종 결과를 데이터베이스에 삽입합니다.
        final_df.to_sql(
            f"statistics_{market}", con=connection, if_exists="append", index=False
        )
