...
def _update_similar_data(base_date, market, day_num, code_list, prepare=True):
    ...
    if day_num in [16, 32, 64, 128]:
        if prepare:
            change, no_change = prepared_data.classify_is_hi_lo_changed(
                base_date, market, day_num, code_list, only_close=only_close
            )
            similar, invalid_code = prepared_data.search_prepared_similar_data(
                base_date, market, day_num, code_list=no_change, only_close=only_close
            )
            similar = similar | direction_method.get_similar_data(
                base_date,
                market,
                day_num,
                code_list=change + invalid_code,
                only_close=only_close,
            )
        else:
            similar = direction_method.get_similar_data(
                base_date, market, day_num, code_list=code_list, only_close=only_close
            )
    elif day_num == 8:
        similar = candle_color_method.get_similar_data(
            base_date, market, day_num, code_list=code_list, batch=15
        )
    ...
    for base_stock_code in code_list:
        ...
        for similar_chart_data in similar[base_stock_code]:
            distance, compare_stock_code, compare_date, after_close_change = (
                similar_chart_data
            )
            ...
            for key in list(min_distance_data.keys()):
                ...
                if (
                    stored_compare_code == compare_stock_code
                    and abs((compare_date - stored_compare_date).days) <= 10
                ):
                    # if the new distance is greater than the stored distance
                    if distance < min_distance_data[key][4]:
                        # update the data
                        min_distance_data[key] = (
                            base_stock_code,
                            base_date,
                            compare_stock_code,
                ...
                min_distance_data[(compare_stock_code, compare_date)] = (
                    base_stock_code,
                    base_date,
                    compare_stock_code,
                    compare_date.strftime("%Y-%m-%d"),
    ...
    data_to_insert.to_sql(
        f"comparison_result_{day_num}day_{market}",
        con=engine,
        if_exists="append",
        index=False,
    )
...
def update_all_similar_data(start_time, market, batch=10000):
    ...
    for rank in range(0, len(all_code_list), batch):
        ...
        for day_num in get_day_num_list():
            ...
            logging.info(
                f"update_similar_data {day_num}day {start} ~ {end} done {datetime.now() - start_time}"
            )
...
def generate_all_normed_data(market, start_time, period=120):
    ...
    direction_method.generate_normed_data(
        start_date, end_date, market, 128, batch=30, prepare=True
    )
    direction_method.generate_normed_data(
        start_date, end_date, market, 64, batch=30, prepare=True
    )
    direction_method.generate_normed_data(
        start_date, end_date, market, 32, batch=30, prepare=True
    )
    direction_method.generate_normed_data(
        start_date, end_date, market, 16, batch=30, prepare=True
    )
