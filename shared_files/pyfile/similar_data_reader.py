...
def get_one_comparison_result(
    base_stock_code, base_date, compare_stock_code, compare_date, day_num, market
):
    engine = get_engine()

    query = text(
        f"""SELECT * from comparison_result_{day_num}day_{market}
...
def get_similar_data_range(code, day_num, market):
    ...
    if data_range_str is None:
        query = text(
            f"SELECT DISTINCT base_date FROM comparison_result_{day_num}day_{market} WHERE base_stock_code = :code"
        )
        ...
        data_range_list = sorted(
            data_range_df["base_date"].dt.strftime("%Y-%m-%d").tolist()
        )
