...
def get_average_all_score(market, update_cache=False):
    ...
    if average_score is not None and not update_cache:
        ...
        query = text(f"""SELECT AVG(score) AS average_score FROM profit_after_{VALIDATION_DAY}day_{market}""")
...
def get_average_dif(date, market, update_cache=False):
    ...
    if average_dif is not None and not update_cache:
        ...
        average_dif = get_average_score(date, market, update_cache) - get_average_all_score(market, update_cache)
...
def insert_new_data(date, market):
    ...
    # 교집합 코드 찾기
    common_codes = set(all_statistics['code']).intersection(set(all_close_price['code']))
    ...
    all_statistics['rise_rate_allday'] = round(all_statistics['rise_count_allday'] / all_statistics['data_num_allday'] * 100, 1)

    # score 계산
    all_statistics['score'] = all_statistics.apply(
        lambda row: round(max(min(
                    max(min(row['average_allday'] * 0.7, 5), -5) + (row['rise_rate_allday'] - 50) * 0.2, 
                    10.0), -10.0), 1), axis=1)
    ...
    # 데이터베이스에 데이터 삽입
    engine = get_engine()
    insert_data.to_sql(f'profit_after_{VALIDATION_DAY}day_{market}', con=engine, if_exists='append', index=False)
...
def validate_profit(date, market):
    ...
    with engine.begin() as conn:
        ...
        for code in missing_codes:
            ...
            if stock_data.empty:
                logging.warning(f"No data found for {code} on {past_date}. Removing from dataset.")
                df = df[df['code'] != code]  # 데이터가 없으면 해당 행 삭제
            else:
                close_price = stock_data['Close'].iloc[0]
                df.loc[df['code'] == code, 'after_close_price'] = close_price

        df['base_close_price'] = df['code'].map(past_close_price_dict).fillna(df['base_close_price'])
        df['profit'] = ((df['after_close_price'] - df['base_close_price']) / df['base_close_price'] * 100).where(pd.notna(df['after_close_price']), None)

        # 기존 데이터 삭제
        delete_stmt = text(f"DELETE FROM profit_after_{VALIDATION_DAY}day_{market} WHERE date = '{past_date}'")
        conn.execute(delete_stmt)

        # 데이터를 다시 삽입
        df.to_sql(f'profit_after_{VALIDATION_DAY}day_{market}', con=conn, if_exists='append', index=False)

    if market == 'nyse_naq':
        update_snp500_profit_validation(past_date)
    logging.info(f"validate profit done for {market} / {date} - {VALIDATION_DAY}days ago : {past_date}")
...
def get_validation_allday(market, lang, type, cache_update=False):
    ...
    if data and not cache_update:
        ...
        for i, date in enumerate(unique_dates):
            d_n_value = f"D-{i+1}"
            data.loc[data['date'] == date, 'profit'] = data.loc[data['date'] == date, 'profit'].apply(
                lambda x: d_n_value if pd.isna(x) else round(x, 2))
...
def _get_latest_range_total_counts(target_date, engine):
    ...
    """)
    prev_date_result = pd.read_sql(query_prev_day, engine, params={'target_date': target_date})
...
def insert_snp500_profit_validation(target_date):
    ...
    for (low, high), label in zip(SCORE_RANGES, SCORE_RANGE_LABELS):
        if low == '-inf':
            ...
            filtered = current_data[(current_data['score_plus_avg'] >= low) & (current_data['score_plus_avg'] <= high)]
    ...
    result_df.to_sql('snp500_profit_validation', engine, if_exists='append', index=False)
...
def update_snp500_profit_validation(target_date):
    ...
    for (low, high), label in zip(SCORE_RANGES, SCORE_RANGE_LABELS):
        ...
        old_average_profit = prev_data[label]['average_profit'] if prev_data[label]['average_profit'] is not None else 0
        ...
        if new_range_total_num > 0:
            new_average_profit = (old_average_profit * old_range_total_num + sum_new_profits) / new_range_total_num
        else:
    ...
    for label in SCORE_RANGE_LABELS:
        if not any(res['score_range'] == label for res in results):
            results.append({
                ...
                'average_profit': prev_data[label]['average_profit'] if prev_data[label]['average_profit'] is not None else 0,
                'all_stock_code': '',
                'rise_num': 0,
                'rise_stock_code': '',
                'fall_stock_code': '',
...
def get_total_validation(cache_update=False):
    ...
    if data is None or cache_update:
        ...
        # 역전된 5개의 행과 나머지 데이터프레임을 결합
        data = pd.concat([reversed_top5, data.iloc[MINUS_LABEL_NUM:]]).reset_index(drop=True)
...
def get_all_validation_date(cache_update=False):
    ...
    if df_dates is None or cache_update:
        query = f"SELECT DISTINCT date FROM snp500_profit_validation ORDER BY date DESC;"
        df_dates = pd.read_sql(query, engine)
        ...
        df_dates = pd.read_json(io.StringIO(df_dates.decode('utf-8')), dtype={'date': str})
