...
def draw_all_after_change_chart(code, market, lang, statistics_data, validation_date=''):
    if validation_date != '':
    ...
    for day_num in get_day_num_list():
        ...
        if after_change is None:
            continue
        future_change = last_close * (1 + after_change * 0.01)  # 종가에 변동률을 적용한 미래 가격

        # 현재 종가에서 미래 가격을 연결하는 선 그리기
        axes[0].plot([last_index, future_index], [last_close, future_change], label=f'{day_num}', alpha=0.5, linewidth=1)
    ...
    if after_change is None:
        ...
        axes[0].plot([last_index, future_index], [last_close, average_change], color=color_up if after_change >= 0 else color_down, linewidth=3.0, 
                    marker='o', label=("평균" if lang == "ko" else "Average"), alpha=0.8)
        
        # 선에 대한 설명 달기
        axes[0].legend(loc='upper left')

        average_text = "{:.2f}".format(statistics_data[f'average_allday']) + ('% ▲' if 0 <= statistics_data[f'average_allday'] else '% ▼')
        axes[0].text(future_index + 2.5, average_change, average_text, fontsize=12,
            verticalalignment='center', color=color_up if after_change >= 0 else color_down, fontweight='bold')

    # 거래량 차트 그리기
    axes[1].bar(x, base_data['Volume'], color='grey', width=0.65, align='center')
    axes[1].set_xticks(range(len(x)))

    base_dates = base_data.index.strftime('%m.%d') # 첫 번째 값과 마지막 값을 포함해 총 6개의 날짜를 선택
...
def _draw_all_after_change_chart2(code, market, lang, statistics_data, validation_date=''):
    if validation_date != '':
    ...
    for day_num in get_day_num_list():
        ...
        # 현재 종가에서 미래 가격을 연결하는 선 그리기
        ax.plot([last_index, future_index], [last_close, future_change], label=f'{day_num}', alpha=0.5, linewidth=2)
    ...
    if after_change is None:
        ...
        after_change = statistics_data[f'average_allday'] 
        average_change = last_close * (1 + after_change * 0.01)
        ax.plot([last_index, future_index], [last_close, average_change], color=color_up if after_change >= 0 else color_down, linewidth=7, 
                    marker='o', alpha=0.8)
...
def _draw_all_data_num_chart(code, market, lang, statistics_data, validation_date=''):
    ...
    plt.pie(sizes, colors=colors, autopct=lambda pct: f'{pct:.0f}%\n', startangle=90, textprops={'fontsize': 32, 'fontweight': 900, 'color': 'white'})
        ...
        if only_close:
            ...
            for x in range(x_start, x_end):
                if close_prices[day] > open_prices[day]:
                    ...
                    tail_color = [200, 0, 0] if lang == 'ko' else [0, 140, 0] # 상승 시 꼬리: 어두운 빨강
                elif close_prices[day] < open_prices[day]:
                    color = [0, 0, 255] if lang == 'ko' else [255, 0, 0] # 하락: 파랑
                    tail_color = [0, 0, 200] if lang == 'ko' else [200, 0, 0] # 하락 시 꼬리: 어두운 파랑
        ...
        if 0 < day and not only_close:
            ...
            if not np.isnan(ma20_norm) and not np.isnan(stock_data_ma20.iloc[idx-1]):
                ...
                if 0 <= ma20_y < height:
                    ...
                    # 선형 보간을 위한 x, y 값 설정
                    x = np.array([x_start_orig - adjust_width2, x_end_orig - adjust_width2]) # 시작과 끝 지점을 day의 반만큼 왼쪽으로 이동
                    ...
                    # 보간할 x값 생성
                    xnew = np.arange(x_start_orig - adjust_width2, x_end_orig - adjust_width2, 1)
...
def draw_compare_chart(code, base_date, similar_chart, market, day_num, lang):
    ...
    img_top_center = convert_image(get_stock_data_pre_fol(code, base_date, market, preceding=day_num + 18), day_num, lang)
    ...
    ax_top_center.set_xlabel(('기존 차트' if lang == "ko" else 'original'), fontsize=11, fontweight='bold')
    ...
    for cnt, (i, row) in enumerate(similar_chart.iterrows()):
        similar_data = get_stock_data_pre_fol(row['compare_stock_code'], row['compare_date'], market, preceding=day_num + 18)
        ...
        if len(img) > 0:
            ...
            text = get_name(row['compare_stock_code'], market, lang='ko') if market == 'kospi_daq' else row['compare_stock_code']
            ax.set_xlabel(f"{cnt + 1}. {text}\n{similar_data.index[0].strftime('%y.%m.%d')}~{similar_data.index[-1].strftime('%y.%m.%d')}", fontsize=9, fontweight='bold')
        else:
    ...
    # Create legend
    legend_elements = [Line2D([0], [0], color=[139/255, 0, 255/255], lw=3, label='5'),
    ...
    plt.savefig(f'/app/shared_files/static/image_data/{market}/{base_date}/compare_chart_{code}_{base_date}_{str(day_num)}_{lang}.png', dpi=200, bbox_inches='tight', pad_inches=0.1)
    plt.clf()
    plt.close("all")

# result 페이지의 향후 변동의 평균을 표기한 이미지 생성
def draw_expected_chart(code, base_date, similar_data, base_data, market, day_num, lang):
    ...
    for cnt, (i, row) in enumerate(similar_data.iterrows()):
        ...
        after_change = get_stock_data_pre_fol(row['compare_stock_code'], row['compare_date'], market, following=get_predict_day(day_num))
        if len(after_change) == 0:
            continue
        after_change = after_change['Change'].iloc[1:]
            
        predicted_prices = [last_close] + [last_close * change for change in np.cumprod(1 + np.array(after_change))]
        future_x = range(len(base_data.index) - 1, len(base_data.index) - 1 + len(predicted_prices))
        if cnt != similar_chart_num - 1:
            axes[0].plot(future_x, predicted_prices, color='grey', alpha=0.3)
        else:
            axes[0].plot(future_x, predicted_prices, color='grey', alpha=0.3, label=("향후 추이" if lang == "ko" else "Future Trends"))
    ...
    predicted_prices_avg = [last_close] + [last_close * change for change in np.cumprod(1 + np.array(average_change))]
    # Add predicted prices to the chart with a red thick line
    future_x_avg = range(len(base_data.index) - 1, len(base_data.index) - 1 + len(predicted_prices_avg))
    after_close_mean = round(similar_data['after_close_change'].mean(), 2)
    if after_close_mean < 0:
        axes[0].plot(future_x_avg, predicted_prices_avg, color=color_down, linewidth=2.0, alpha=0.5, label=("평균 향후 추이" if lang == "ko" else "Average Future Trends"))
    else:
        axes[0].plot(future_x_avg, predicted_prices_avg, color=color_up, linewidth=2.0, alpha=0.5, label=("평균 향후 추이" if lang == "ko" else "Average Future Trends"))
    ...
    plt.savefig(f'/app/shared_files/static/image_data/{market}/{base_date}/expected_chart_{code}_{base_date}_{str(day_num)}_{lang}.png', bbox_inches='tight', pad_inches=0.1)
    plt.clf()
    plt.close("all")
...
def draw_detail_chart(code, base_date, market, day_num, lang):
    ...
    data = get_stock_data_pre_fol(code, base_date, market, preceding=day_num + 18, following=predict_day)
    ...
    fig.update_layout(xaxis_rangeslider_visible=False, dragmode=False, autosize=True, margin=dict(l=0, r=0),
    ...
    hover_texts = [f"<b>Date:</b> {date}<br>{change}" for date, change in zip(date_strings, change_percentage)]
    ...
    fig.add_trace(go.Bar(x=x_values, y=base_data['Volume'], showlegend=False, marker=dict(color='#808080')), row=2, col=1)
    ...
    fig.add_trace(go.Scatter(x=x_values, y=base_data['MA5'], mode='lines', name='5', line=dict(color='rgba(139, 0, 255, 0.5)'), hoverinfo='skip'))
    fig.add_trace(go.Scatter(x=x_values, y=base_data['MA10'], mode='lines', name='10', line=dict(color='rgba(96, 96, 96, 0.5)'), hoverinfo='skip'))
    fig.add_trace(go.Scatter(x=x_values, y=base_data['MA20'], mode='lines', name='20', line=dict(color='rgba(252, 190, 0, 0.5)'), hoverinfo='skip'))
    ...
    fig.add_shape(type="rect", xref="x", yref="paper", x0=start_of_yellow, y0=0, x1=end_of_yellow, 
                  y1=1, fillcolor="yellow", opacity=0.2, layer="below", line_width=0)
    trend = '향후 추이' if lang == 'ko' else 'Future Trends'
    fig.add_trace(go.Scatter(x=[None], y=[None], mode='markers', name=trend, marker_symbol='square', marker=dict(color='yellow'), hoverinfo='skip'))
...
def draw_statistics_chart(data, market, day_num, file_name):
    for lang in get_lang_list():
        ...
        # Add an image to the new row at the third column
        ax_top_center = axs[0][2]
        img_top_center = convert_image(get_stock_data_pre_fol(data.iloc[0]['code'], data.iloc[0]['date'], market, preceding=day_num + 18), day_num, lang)
        ...
        ax_top_center.set_xlabel((get_name(data.iloc[0]['code'], market, lang='ko') if market == 'kospi_daq' else data.iloc[0]['code']), fontsize=13, fontweight='bold')
        ...
        for cnt, (i, row) in enumerate(data.iterrows()):
            stock_data = get_stock_data_pre_fol(row['code'], row['date'], market, preceding=day_num + 18)
            ...
            if len(img) > 0:
                ax.imshow(img)
                text = get_name(row['code'], market, lang='ko') if market == 'kospi_daq' else row['code']
                if lang == 'ko':
                    color = 'red' if row['average'] >= 0 else 'blue'
                else:
                    color = 'green' if row['average'] >= 0 else 'red'
                ax.set_xlabel(f"{cnt + 1}. {text}", fontsize=10, fontweight='bold', color=color)
            else:
                ax.imshow(np.ones((1, 1, 3), dtype=np.uint8) * 255) # 그냥 흰 이미지
        ...
        axs[0][0].text(0.5, 0.8, f"{stock_data.index[0].strftime('%y.%m.%d')}~{stock_data.index[-1].strftime('%y.%m.%d')}", fontsize=10, fontweight='bold', ha='center', va='center')
        ...
        plt.savefig(f'/app/shared_files/static/image_data/statistics/{file_name}_{lang}.png', dpi=200, bbox_inches='tight', pad_inches=0.1)
        plt.clf()
        plt.close("all")
...
def draw_stock_info_charts(code, base_date, market, lang):
    ...
    for day in get_day_num_list(type='str'):
        if not os.path.exists(f'/app/shared_files/static/image_data/cache/{code}_{base_date}_{day}_{lang}.png'):
            all_exist = False
    if all_exist:
        return
    
    for day_num in get_day_num_list():
        stock_data = get_stock_data_pre_fol(code, base_date, market, preceding=day_num + 18)
        img = convert_image(stock_data, day_num, lang)
        if len(img) == 0:
            img = np.ones((1, 1, 3), dtype=np.uint8) * 255
        img = Image.fromarray(img, 'RGB')
        img.save(f'/app/shared_files/static/image_data/cache/{code}_{base_date}_{day_num}_{lang}.png')
...
def draw_drawing_search_chart(code, base_date, day_num, market, lang, pattern=False):
    ...
    if pattern:
        stock_data = get_stock_data_pre_fol(code, base_date, market, preceding=day_num+16, following=2)
    else:
        stock_data = get_stock_data_pre_fol(code, base_date, market, preceding=day_num+18)
...
def draw_plt_stock_chart(code, base_date, lang, stock_data, validation_chart=False):
    ...
    if not validation_chart:
        ...
        axes[0].legend(handles=[legend_patch1, ], loc='upper left')
    else:
        for i in range(len(stock_data) - 1, len(stock_data) - 11, -1):
            axes[0].axvspan(i-0.5, i+0.5, facecolor=light_green, edgecolor='none')
        axes[0].axvspan(base_index-0.5, base_index+0.5, facecolor=light_blue, edgecolor='none')

        legend_patch1 = Patch(facecolor=light_green, edgecolor='none', label='향후 추이' if lang == 'ko' else 'Future Trends')
        legend_patch2 = Patch(facecolor=light_blue, edgecolor='none', label='평가일' if lang == 'ko' else 'Evaluation Date')
    ...
    base_dates = stock_data.index.strftime('%m.%d') # 첫 번째 값과 마지막 값을 포함해 총 6개의 날짜를 선택
    indices = np.linspace(0, len(base_dates) - 1, 6, dtype=int)
    x_labels = [base_dates[i] if i in indices else "" for i in range(len(base_dates))]
    axes[1].set_xticklabels(x_labels, fontsize=9)
    axes[1].get_yaxis().set_visible(False)
...
def update_index_image(data, market):
    for lang in ['ko', 'en']:
        ...
        img.save(f'/app/shared_files/static/image_data/{market}/{file_name}_chart_{lang}.png')