...
from pyfile.statistics_reader import (
    get_statistics_one_stock,
    get_statistics_stocks,
    get_valid_statistics_list,
)
...
def update_cache_stock_code(market, valid=True):
    ...
    if valid:
        ...
        df_code = pd.read_sql(
            f"SELECT code, ranking FROM stock_code_list_{market} WHERE valid=1 ORDER BY ranking",
            engine,
        )
        ...
        df_code = pd.read_sql(
            f"SELECT * FROM stock_code_list_{market} ORDER BY ranking", engine
        )
...
def get_stock_code(market, only_code=False, only_valid=True, cache_update=False):
    ...
    if only_valid:
        ...
        if data_json is None or cache_update:
            ...
            df_code = pd.read_json(
                io.StringIO(data_json.decode("utf-8")), dtype={"code": str}
            )
...
def update_cache_stock_name(market):
    ...
    query = text(
        f"""UPDATE stock_name_{market} SET name_{another_lang} = name_{lang} 
...
def get_all_name(market, only_valid=True):
    ...
    if only_valid:
        return df_name[
            df_name["code"].isin(
                get_stock_code(market, only_valid=True, only_code=True)
            )
        ]
    else:
        return df_name
...
def update_cache_stock_industry(market):
    ...
    query = text(
        f"""UPDATE stock_industry_{market} SET industry_en = industry_ko 
    ...
    df_industry = pd.read_sql(
        f"""SELECT * FROM stock_industry_{market} WHERE code IN 
...
def get_industry(code, market, lang):
    ...
    if data_json is None:
        ...
        df_industry = pd.read_json(
            io.StringIO(data_json.decode("utf-8")), dtype={"code": str}
        )

    try:
        return df_industry.loc[df_industry["code"] == code, f"industry_{lang}"].values[
            0
        ]
    except IndexError:
...
def update_cache_investing_url(market):
    ...
    df_investing_url = pd.read_sql(
        f"""SELECT * FROM investing_info_{market} WHERE code IN 
                          (SELECT code FROM stock_code_list_{market} WHERE valid=1)""",
        engine,
        dtype={"code": str, "url": str},
    )
...
def get_same_industry_code(code, market, num=11, cache_update=False):
    ...
    if same_industry is not None:
        same_industry = pd.read_json(
            io.StringIO(same_industry.decode("utf-8")), dtype={"code": str}
        )
    else:
        ...
        same_industry_codes = df_industry[df_industry["industry_ko"] == industry][
            "code"
        ].tolist()
        ...
        if len(same_industry_codes) < num:
            ...
            if len(other_codes) >= num - len(same_industry_codes):
                same_industry_codes += random.sample(
                    other_codes, num - len(same_industry_codes)
                )

        # 시가총액 순위로 정렬
        df_code = get_stock_code(market)[["code", "ranking"]]
        same_industry = df_code[df_code["code"].isin(same_industry_codes)].sort_values(
            by="ranking"
        )
        ...
        same_industry["average"] = same_industry["code"].apply(
            get_statistics_one_stock, args=(market, True)
        )
...
def update_cache_naver_url(market):
    ...
    df_naver_url = pd.read_sql(
        f"""SELECT * FROM naver_info_{market} WHERE code IN 
                          (SELECT code FROM stock_code_list_{market} WHERE valid=1)""",
        engine,
    )
...
def get_comparable_code_list(market, cache_update=False):
    ...
    if code_list is None or cache_update:
        code_list = pd.read_sql(
            f"""SELECT code FROM stock_data_{market} WHERE code in 
                                (SELECT code from stock_code_list_{market} WHERE valid = 1) 
                                GROUP BY code HAVING COUNT(*) >= 8""",
            engine,
        )["code"].to_list()
        redis.set(key, json.dumps(code_list))
...
def get_up_down_ranking(market, lang="ko", cache_update=False):
    ...
    for change in ["up", "down"]:
        ...
        if data is None or cache_update:
            ...
            code_list = data[data["code"].isin(get_valid_statistics_list(market))][
                "code"
            ].tolist()[:10]
            if len(code_list) == 0:
                return
