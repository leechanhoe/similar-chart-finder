...
from pyfile.data_reader import (
    get_investing_url_all,
    get_all_name,
    get_all_industry,
    get_stock_code,
)
from pyfile.data_reader import (
    update_cache_stock_name,
    update_cache_stock_industry,
    update_cache_investing_url,
    update_cache_naver_url,
)
...
def update_popular_stock(market):
    ...
    if market == "kospi_daq":
        ...
        }  # investing.com은 헤더가 없으면 접근을 제한합니다.
        try:
...
def update_investing_url(code_list, market, recreate=False):
    ...
    category = (
        ["주식 - 서울 equities", "주식 - 코스닥 equities", "주식 - 코넥스 equities"]
        if market == "kospi_daq"
        else ["주식 - 나스닥 equities", "주식 - 뉴욕 equities"]
    )
        ...
        for _ in range(10):
            try:
                ...
                search_result = soup.find(
                    "div",
                    class_="js-inner-all-results-quotes-wrapper newResultsContainer quatesTable",
                )
                if search_result is not None:  # None 검사 추가
                    ...
                    a = [
                        tag
                        for tag in a
                        if tag.find("span", class_="fourth").get_text() in category
                    ]
                    if a:  # None 검사 추가
...
def update_translated_name(market):
    ...
    base_url = (
        "https://investing.com"
        if market == "kospi_daq"
        else "https://m.stock.naver.com/worldstock/stock/"
    )
    headers = {
        "User-Agent": "Mozilla/5.0"
    }  # investing.com은 헤더가 없으면 접근을 제한합니다.
    ...
    if null_rows.shape[0] > 0:
        raise Exception(
            "Null row(s) exist in the 'code' column of the df_name dataframe", null_rows
        )
            ...
            for i in range(2):
                try:
                    ...
                    if result is not None:  # None 검사 추가
                        ...
                        if len(new_name) == 0:
                            logging.warning(
                                f"{idx}. {row['code']}({row[f'name_{lang}']})'s name_{lang_another} is blank!"
                            )
                        else:
                            df_name.at[idx, f"name_{lang_another}"] = new_name
                            logging.info(
                                f"{idx}. update name_{lang_another} : {row['code']} to {new_name}"
                            )
                            break
                    else:
                    ...
                    logging.info(
                        f"Request to {url} failed, retrying {i} in 1 seconds..."
                    )
                    time.sleep(0.3)  # 잠시 대기 후 재시도합니다.
    ...
    with engine.begin() as connection:
        update_query = text(
            f"UPDATE {table_name} SET name_{lang_another} = :name_lang_another WHERE code = :code"
        )
        for idx, row in df_name[["code", "name_ko", "name_en"]].iterrows():
            connection.execute(
                update_query,
                {"name_lang_another": row[f"name_{lang_another}"], "code": row["code"]},
            )

        if market == "nyse_naq":  # 탐색하며 얻어낸 naver url도 테이블에 저장
            update_query = text(
                f"""INSERT INTO naver_info_nyse_naq (code, url)
                                    VALUES (:code, :url)
                                    ON DUPLICATE KEY UPDATE url = :url;"""
            )
            for idx, row in df_name[["code", "url"]].iterrows():
                connection.execute(
                    update_query, {"url": row["url"], "code": row["code"]}
                )
...
def update_industry_en(market):
    ...
    with engine.begin() as connection:
        ...
        for i, industry_ko in enumerate(industry_list):
            ...
            connection.execute(
                update_query, {"industry_en": industry_en, "industry_ko": industry_ko}
            )
