...
# 통계 페이지용 데이터 가져오기
def get_statistics(market='kospi_daq', rank=10, up=True, rise_rate=False, day_num="all", update=False):
    engine = get_engine()
    r = get_redis()
    ...
    if data is not None and not update:
        ...
        data = data[['code', 'date', f'average_{day_num}day', f'rise_count_{day_num}day', f'data_num_{day_num}day']]
        data.dropna(inplace=True)
        data.rename(columns={f'average_{day_num}day': 'average', f'rise_count_{day_num}day': 'rise_count', f'data_num_{day_num}day': 'data_num'}, inplace=True)
...
def get_statistics_stocks(code_list, market):
    ...
    df.rename(columns={f'average_allday': 'average', f'rise_count_allday': 'rise_count', 'data_num_allday': 'data_num'}, inplace=True)
