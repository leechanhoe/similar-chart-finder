...
def get_all_date_stock_data(code, market, reupdate=False):
    ...
    code_list = list(
        set(get_stock_code(market, only_code=True))
        | set(get_compared_code_list(market))
    )
    ...
    if price_data is None or reupdate:
        ...
        keys_to_delete = set(
            key.decode("utf-8") for key in raw_keys
        )  # byte 문자열을 utf-8로 디코드
    ...
    price_data = pd.read_json(
        io.StringIO(price_data.decode("utf-8")), dtype={"code": str}
    )
    if len(price_data) == 0:
        return none_stock_data
...
def get_stock_data_start_end(
    code, market, start_date, end_date=pd.Timestamp.now().strftime("%Y-%m-%d")
):
    valid_code_list = list(
        set(get_stock_code(market, only_code=True))
        | set(get_compared_code_list(market))
    )
...
def get_stock_data_pre_fol_all(date, market, preceding=0, following=0, all_date=False):
    ...
    if all_date:
        ...
        df_all_dates = pd.read_sql_query(
            query, engine
        )  # SQL 쿼리 작성 - 모든 종목의 날짜 데이터 가져오기
        ...
        end_date = df_all_dates.iloc[min(len(df_all_dates) - 1, idx + following)][
            "date"
        ]
        ...
        df_range = pd.read_sql_query(
            query_range, engine, params={"start_date": start_date, "end_date": end_date}
        )
