...
def update_code_list(market: str):
    ...
    with engine.begin() as connection:
        ...
        common_stocks_fdr_set = df_codes[
            df_codes["code"].isin(db_stock_data_df["code"])
        ]
        columns_to_update = [
        ...
        for column, table_name in columns_to_update:
            ...
            changed_df = merged_df[
                merged_df[f"old_{column}"] != merged_df[f"new_{column}"]
            ]
...
def update_stock_data(base_date, market, month=130, recreate=False):
    ...
    with engine.begin() as connection:
        ...
        if (
        ...
        ):  # 초기상태 : 데이터베이스에 데이터가 없는상태면 다 넣어주기
            ...
            """
            latest_closing_prices = pd.read_sql_query(
                latest_closing_prices_query, connection
            )
            try:  # fdr.StockListing('KRX')에서 간헐적으로 라이브러리 자체의 오류발생
                ...
                latest_closing_dict = latest_closing_prices.set_index("code").T.to_dict(
                    "records"
                )[0]
                ...
                for idx, row in latest_df.iterrows():
                    ...
                    if row["Code"] in latest_closing_dict:
                        ...
                        if round(close_change * 100) != round(row["ChagesRatio"]):
                            logging.info(
                                f"{row['Code']}에서 변동률 {round(close_change)} 와 {round(row['ChagesRatio'])} 가 다름"
                            )
                            # change 리스트에 추가
                ...
                Exception
            ) as e:  # fdr.StockListing('KRX')오류 발생시 다른 로직으로 데이터 업데이트
                logging.warning(f"fdr.StockListing('KRX') error : \n{e}")

                day_ago_close = latest_closing_prices.set_index("code")[
                    "close_price"
                ].to_dict()
            ...
            for code in reupdate:
                logging.info(
                    f"유상증자, 무상증자, 액면분할로 주가가 변경된 종목 : {code}"
                )
                try:
                    new_data_df = get_stock_data_fdr(
                        code, start_date, end_date, market
                    )  # 일단 11년치 다 불러오기 (어차피 주말에 조정됨)
                except Exception as e:
            ...
            rows_to_insert_today = [
                {
                    ...
                    "date": today,
                    "open_price": (
                        int(row["Close"]) if row["Open"] == 0 else int(row["Open"])
                    ),
                    "high_price": (
                        int(row["Close"]) if row["High"] == 0 else int(row["High"])
                    ),
                    "low_price": (
                        int(row["Close"]) if row["Low"] == 0 else int(row["Low"])
                    ),
                    "close_price": int(row["Close"]),
                    "volume": None if pd.isna(row["Volume"]) else int(row["Volume"]),
                    "change_rate": (
                        None if pd.isna(row["Change"]) else float(row["Change"])
                    ),
                }
                for idx, row in latest_df.iterrows()
            ]
